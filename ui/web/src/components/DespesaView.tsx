...
import { Search, Plus, Pencil, Trash2, Zap, Anchor, RefreshCw, ArrowLeft, SlidersHorizontal, ArrowUp, ArrowDown, ChevronsUpDown } from 'lucide-react';
...
import {
  ...
  PERIODICIDADE_LABELS, TIPO_DESPESA_LABELS,
} from '@/lib/types';
...
export function DespesaView({ highlightId, onHighlightConsumed }: DespesaViewProps = {}) {
  ...
  const [filtros, setFiltros] = useState<DespesaFiltros>(FILTROS_VAZIOS);
  ...
  const filtered = useMemo(() =>
    (despesas ?? [])
      ...
      .filter(d => {
        if (filtros.estado === 'todas' || d.tipoDespesa === 'Pontual') return true;
        return filtros.estado === 'ativa' ? d.isActive : !d.isActive;
      })
      .filter(d => {
        if (filtros.pagamento === 'todas') return true;
        if (filtros.pagamento === 'pendente') return d.parcelas.some(p => !p.isPaid);
        return d.parcelas.length > 0 && d.parcelas.every(p => p.isPaid);
      })
      .filter(d => {
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                <PillButton active={filtros.tipo === 'todos'} onClick={() => setFiltros(f => ({ ...f, tipo: 'todos' }))}>Todos</PillButton>
                {(['Pontual', 'Recorrente', 'Fixa'] as TipoDespesa[]).map(tipo => (
                  <PillButton key={tipo} active={filtros.tipo === tipo} onClick={() => setFiltros(f => ({ ...f, tipo }))}>
                    {TIPO_DESPESA_LABELS[tipo]}
                  </PillButton>
                ))}
              </div>
            </div>
            ...
            <div>
              <Label className="text-xs font-semibold text-slate-500 uppercase tracking-wide">
                Estado <span className="normal-case font-normal text-slate-400">(Fixa/Recorrente)</span>
              </Label>
              <div className="flex gap-1.5 mt-1.5">
                <PillButton active={filtros.estado === 'todas'} onClick={() => setFiltros(f => ({ ...f, estado: 'todas' }))}>Todas</PillButton>
                <PillButton active={filtros.estado === 'ativa'} onClick={() => setFiltros(f => ({ ...f, estado: 'ativa' }))}>Ativa</PillButton>
                <PillButton active={filtros.estado === 'inativa'} onClick={() => setFiltros(f => ({ ...f, estado: 'inativa' }))}>Inativa</PillButton>
              </div>
            </div>
